...
public class DepartmentService implements IDepartmentService{
	...
	public List<DepartmentDTO> findAllByCityName(String cityName) {
        ...
        public Map<String, Long> getroom_type() {
            List<DepartmentEntity> departments = departmentRep.findAll();
            return departments.stream()
                              .collect(Collectors.groupingBy(DepartmentEntity::getRoomType, Collectors.counting()));
        }
		...
		public List<DepartmentDTO> findFilteredDepartments(String roomType, Integer maxPerson) {
			Specification<DepartmentEntity> spec = Specification.where(roomTypeEquals(roomType))
																.and(maxPersonLessThanOrEqualTo(maxPerson));
			
			List<DepartmentEntity> entities = departmentRep.findAll(spec);
			return entities.stream().map(DepartmentConvert::toDto).collect(Collectors.toList());
		}

		private Specification<DepartmentEntity> roomTypeEquals(String roomType) {
			return (root, query, criteriaBuilder) -> {
				if (roomType == null || roomType.isEmpty()) return criteriaBuilder.conjunction();
				return criteriaBuilder.equal(root.get("roomType"), roomType); // Sử dụng "roomType", không phải "room_type"
			};
		}
		private Specification<DepartmentEntity> maxPersonLessThanOrEqualTo(Integer maxPerson) {
			return (root, query, criteriaBuilder) -> {
				if (maxPerson == null) return criteriaBuilder.conjunction();
				return criteriaBuilder.greaterThanOrEqualTo(root.get("max_person"), maxPerson);
			};
		}
}
