...
public class Web {
	...
	@GetMapping("/roomlist")
	public String getRoomList(@RequestParam(required = false) String roomType,
							  @RequestParam(required = false) Integer maxPerson,
							  Model model) {
		if (roomType != null || maxPerson != null) {
			List<DepartmentDTO> filteredDepartments = departmentService.findFilteredDepartments(roomType, maxPerson);
			model.addAttribute("findAllDepartments", filteredDepartments);
		} else {
			model.addAttribute("all_depart", departmentService.findAllDepartments());
			model.addAttribute("room_type_all", departmentService.getroom_type());
			model.addAttribute("categories", departmentService.getUniqueCategories());
		}
		return "web/blog";
	}
